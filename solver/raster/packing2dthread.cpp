...
void Packing2DThread::changeKeepAspectRatio(int &curLenght, int &curHeight, const qreal ratio)
{
	...
	if (!checkShrinkSizeConstraint(curLenght, curHeight, reducedLength, reducedHeight, ratioSqr))
		return;
...
void Packing2DThread::randomChangeContainerDimensions(int &curLenght, int &curHeight, const qreal ratio)
{
	...
	{
		if (changeLenght)
			curLenght = std::ceil(ratio * (qreal)curLenght);
		else
			curHeight = std::ceil(ratio * (qreal)curHeight);
	}
	else
	{
		int reducedLength, reducedHeight;
		if (!checkShrinkSizeConstraint(curLenght, curHeight, reducedLength, reducedHeight, ratio))
			return;
		if (changeLenght)
			curLenght = reducedLength;
		else
...
bool Packing2DThread::checkShrinkSizeConstraint(int &curLength, int &curHeight, int &reducedLength, int &reducedHeight, qreal ratio)
{
	...
	if (!isPossibleToReduceLength && isPossibleToReduceHeight)
		curHeight = reducedHeight;
	if (!isPossibleToReduceHeight && isPossibleToReduceLength)
		curLength = reducedLength;
	if (!isPossibleToReduceLength || !isPossibleToReduceHeight)
		return false;
	return true;
}
...
void Packing2DThread::runSquare()
{
	...
	bool success = false;
	int curDim;
	qreal rdec = parameters.getRdec();
	qreal rinc = parameters.getRinc();
	...
	while (QDateTime::currentDateTime().msecsTo(finalTime) / 1000.0 > 0 && (parameters.getIterationsLimit() == 0 || totalItNum < parameters.getIterationsLimit()) && !m_abort)
	{
		...
		if (!parameters.isFixedLength())
		{
			...
			if (success)
			{
				...
				curDim = qRound(areaDec * solver->getCurrentWidth()) < curDim ? qRound(areaDec * solver->getCurrentWidth()) : curDim - 1;
				lastFeasibleSolution = QPair<RASTERVORONOIPACKING::RasterPackingSolution, int>(threadSolution, curDim);
			}
			else if (numLoops >= MAXLOOPSPERLENGTH)
			{
				...
				else
				{
					...
					if (curDim >= minSuccessfullSol.length)
					{
						...
						if (curDim >= minSuccessfullSol.length)
							curDim = minSuccessfullSol.length - 1;
					}
				}
			}
			else
...
void Packing2DThread::runRectangle()
{
	...
	curLenght = solver->getCurrentWidth();
	curHeight = solver->getCurrentHeight();
	...
	while (QDateTime::currentDateTime().msecsTo(finalTime) / 1000.0 > 0 && (parameters.getIterationsLimit() == 0 || totalItNum < parameters.getIterationsLimit()) && !m_abort)
	{
		...
		if (success)
		{
			...
			if (parameters.getRectangularPackingMethod() == RASTERVORONOIPACKING::RANDOM_ENCLOSED)
				randomChangeContainerDimensions(curLenght, curHeight, areaDec);
			else if (parameters.getRectangularPackingMethod() == RASTERVORONOIPACKING::COST_EVALUATION)
				...
				changeKeepAspectRatio(curLenght, curHeight, areaDec);
		}
		else if (numLoops >= MAXLOOPSPERLENGTH)
		{
			numLoops = 1;
			...
			{
				if (areaInc * (qreal)curLenght * (qreal)curHeight > minSuccessfullSol.area)
					bagpipeChangeContainerDimensions(curLenght, curHeight);
				else
