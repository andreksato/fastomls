...
// ==========================================================================================
//
// Lista de alteracoes em pbarea.h:
//
// - TSEL: modificado operador < para corrigir erro de assercao (mudanca de <= para <)
...
public:
	...
	void	Sweep(SEGM2 * aSegms, UINT nSegms);
...
protected:
	...
	{
		// data members
		UINT	id;		// event id, must be unique for each event
		// FIX
		pbINT	x, y;	// (rounded) coordinate
	...
	UINT			m_nId;	// maximimum event id

	void	CollectEvents(SEGM2 * aSegms, UINT nSegms);
	...
	inline void Intercept(LIST_TSEL * list, SEGM2 * segm, pbINT cx);
	inline void InsNewNode(SEGM2 * s, TSEL * tsel, pbINT X);
