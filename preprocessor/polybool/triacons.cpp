...
TRAP *split_two_trap(TRAP *t, TRAP *tn, const GRID2 & v0, const GRID2 & v1)
{
    ...
    if ((pbDINT)(t->lo->y - v0.y) * (v1.x - v0.x) >
		(pbDINT)(t->lo->x - v0.x) * (v1.y - v0.y))
    { /* intersecting dl */
        dl->u1 = dr->u0 = tn;
        t->d1  = NULL;
        return dl;
...
void TRIAGLOBS::ConstructTrapezoids()
{
    ...
    for (i = 0; i < n; i++)
	{
		UINT    m = ((UINT)(rand() * 0xffff + rand())) % n;
        VNODE2   *t;
